...
export class GameService {
  private readonly wordListService = inject(WordListService);
  ...
  getRandomWords(count?: number): void {
    ...
    this.letterStatus = this.words.map((word) => word.split('').map(() => ({ correct: null })));
  ...
  endGame(): void {
    ...
    if (this.words.length === this.wordsTyped.length) {
      this.statsService.calculateStats(elapsedTime, this.words, this.wordsTyped);
    }
  }
