...
export class StatsService {
  ...
  calculateStats(timeElapsed: number, allWords: string[], wordsTyped: string[]): void {
    if (timeElapsed <= 0) {
    ...
    const rawSpeed = this.calculateRawSpeed(wordComparison.totalLetters, timeElapsed);
  ...
  private compareWords(
    ...
    for (let i = 0; i < allWords.length; i++) {
      ...
      incorrectLetters += targetWord.getLetterNumber() - typedWord.compareLetters(targetWord);
