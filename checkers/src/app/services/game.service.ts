...
export class GameService {
  ...
  private isValidMove(targetRow: number, targetCol: number): boolean {
    // Get row and column from selected piece
    const { row, col } = this.selectedPiece!;
    ...
    // Check if the move is forward for the perspective player
    const forwardMove =
      (this.currentPlayer === 'red' && dy === -1) || (this.currentPlayer === 'black' && dy === 1);
